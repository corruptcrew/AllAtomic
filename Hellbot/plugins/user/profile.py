...
async def getpfp(client: Client, message: Message):
    ...
    if not user.photo:
        return await AllAtomic.error(hell, f"User {user.mention} has no profile pic.")
...
async def setpfp(client: Client, message: Message):
    if not message.reply_to_message:
        return await AllAtomic.delete(message, "Reply to a photo to set as profile pic.")
    ...
    await AllAtomic.delete(hell, "Profile pic updated successfully.")
...
async def setbio(client: Client, message: Message):
    bio = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, "Processing...")
    ...
    await AllAtomic.delete(hell, "Bio updated successfully.")
    await AllAtomic.check_and_log(
        "setbio",
        f"**User:** {message.from_user.mention} (`{message.from_user.id}`)\n\n**Bio:** `{bio}`",
    )
...
async def setname(client: Client, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Pass a name to set.")

    name = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, "Processing...")
    ...
    await AllAtomic.delete(hell, "Name updated successfully.")
    await AllAtomic.check_and_log(
        "setname",
        f"**User:** {message.from_user.mention} (`{message.from_user.id}`)\n\n**Name:** `{name}`",
    )
...
async def setusername(client: Client, message: Message):
    ...
    await AllAtomic.delete(hell, "Username updated successfully.")
    await AllAtomic.check_and_log(
        "setusername",
        f"**User:** {message.from_user.mention} (`{message.from_user.id}`)\n\n**Username:** `{username}`",
    )
...
async def delpfp(client: Client, message: Message):
    ...
    if not profile_pics:
        return await AllAtomic.error(hell, "No profile pics found.")
...
async def gituser(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Pass a github username to search.")
...
HelpMenu("profile").add(
    ...
    "setname", "<new name>", "Set the name of the bot.", "setname AllAtomic"
).add(
    "setusername",
    "<new username>",
    "Set the username of the bot.",
    "setusername AllAtomic",
