...
async def addgatcha(client: Client, message: Message):
    ...
    if not bot:
        return await AllAtomic.delete(message, "Bot not found!")
    if not bot.is_bot:
        return await AllAtomic.delete(message, "This is not a bot!")
...
async def gatchalist(client: Client, message: Message):
    hell = await AllAtomic.edit(message, "`Fetching gatcha bot list ...`")
...
async def gatchainfo(client: Client, message: Message):
    ...
    if await db.is_gachabot(client.me.id, bot.id, chat_id):
        ...
        return await AllAtomic.edit(message, text)

    await AllAtomic.delete(message, "Bot not found in the gatcha bots list.")
