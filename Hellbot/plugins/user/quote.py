...
async def quotely(client: Client, message: Message):
    if not message.reply_to_message:
        return await AllAtomic.delete(message, "Reply to a message to quote it.")
    ...
    hell = await AllAtomic.edit(message, "__Generating quote...__")
    ...
    if not status:
        return await AllAtomic.error(message, f"`{path}`")
