...
async def alive(client: Client, message: Message):
    ...
    if not img:
        if message.from_user.photo:
            ...
            user_pfp = "./Hellbot/resources/images/AllAtomic_logo.png"
            del_path = False
        img = [
...
async def ping(client: Client, message: Message):
    ...
    hell = await AllAtomic.edit(message, "**Pong ~**")
    uptime = readable_time(time.time() - START_TIME)
    ...
    if img:
        ...
        if img.endswith(".mp4"):
            ...
            await hell.delete()
        return
    await AllAtomic.edit(hell, caption, no_link_preview=True)
...
async def history(client: Client, message: Message):
    ...
    await AllAtomic.edit(hell, response.text)
