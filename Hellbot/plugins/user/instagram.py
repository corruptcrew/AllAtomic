...
async def instagramReels(_, message: Message):
    ...
    if not isInstagramLink(query):
        return await AllAtomic.error(hell, "Give a valid instagram reels link.")

    try:
        driver, _ = Driver.get()
        if not driver:
            return await AllAtomic.error(hell, _)
...
async def instagramPost(_, message: Message):
    ...
    if not isInstagramLink(query):
        return await AllAtomic.error(hell, "Give a valid instagram post link.")
...
async def instagramUser(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Give an instagram username to fetch info.")

    BASE_URL = "https://i.instagram.com/api/v1/users/web_profile_info/"

    query = (await AllAtomic.input(message)).replace("@", "").strip()
    hell = await AllAtomic.edit(message, f"**Searching** `{query}` **on instagram**...")
