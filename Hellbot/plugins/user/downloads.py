...
async def download(_, message: Message):
    hell = await AllAtomic.edit(message, "Starting to download...")
    if message.reply_to_message and message.reply_to_message.media:
        ...
        try:
            ...
            return await AllAtomic.error(hell, f"`{e}`", 10)
        ...
        dwl_url = (await AllAtomic.input(message)).split(" ")
        start_time = time.time()
        try:
            ...
            if dl.isSuccessful():
                ...
                await AllAtomic.error(hell, f"**Failed to download** `{len(dwl_url)} url(s)`")
        except Exception as e:
            return await AllAtomic.error(hell, f"`{e}`", 10)
    else:
