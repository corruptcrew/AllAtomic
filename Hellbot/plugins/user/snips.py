...
from . import Config, HelpMenu, custom_handler, db, handler, AllAtomic, on_message
...
async def addsnip(client: Client, message: Message):
    ...
    keyword = (await AllAtomic.input(message)).replace("#", "")
    hell = await AllAtomic.edit(message, f"Saving snip `#{keyword}`")
    ...
    await AllAtomic.delete(hell, f"**📌 𝖭𝖾𝗐 𝖲𝗇𝗂𝗉 𝖭𝗈𝗍𝖾 𝖲𝖺𝗏𝖾𝖽:** `#{keyword}`")
...
async def rmsnip(client: Client, message: Message):
    if len(message.command[0]) < 7:
        if len(message.command) < 2:
            return await AllAtomic.delete(message, "Give a snip note name to remove.")

        keyword = (await AllAtomic.input(message)).replace("#", "")
        hell = await AllAtomic.edit(message, f"Removing snip `#{keyword}`")
        ...
        if await db.is_snip(client.me.id, message.chat.id, keyword.lower()):
            ...
            await AllAtomic.delete(hell, f"**🍀 𝖲𝗇𝗂𝗉 𝖭𝗈𝗍𝖾 𝖱𝖾𝗆𝗈𝗏𝖾𝖽:** `#{keyword}`")
        else:
            await AllAtomic.delete(hell, f"**🍀 𝖲𝗇𝗂𝗉 𝖭𝗈𝗍𝖾 𝖽𝗈𝖾𝗌 𝗇𝗈𝗍 𝖾𝗑𝗂𝗌𝗍𝗌:** `#{keyword}`")
    else:
        hell = await AllAtomic.edit(message, "Removing all filters...")
        await db.rm_all_snips(client.me.id, message.chat.id)
        await AllAtomic.delete(hell, "All snips have been removed.")
...
async def snips(client: Client, message: Message):
    if message.command[0][0] == "g":
        if len(message.command) < 2:
            return await AllAtomic.delete(message, "Give a snip note name to get.")

        keyword = await AllAtomic.input(message)
        hell = await AllAtomic.edit(message, f"Getting snip `#{keyword}`")
        ...
        hell = await AllAtomic.edit(message, "Getting all snips...")
        ...
        if snips:
            ...
            await AllAtomic.delete(hell, "No snip note saved in this chat.")
