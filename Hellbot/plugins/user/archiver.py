...
async def zip_files(_, message: Message):
    if not message.reply_to_message:
        return await AllAtomic.delete(message, "Reply to a message to zip it.")

    media = message.reply_to_message.media
    if not media:
        return await AllAtomic.delete(message, "Reply to a media message to zip it.")

    hell = await AllAtomic.edit(message, "`Zipping...`")
    start = time.time()
    ...
    await AllAtomic.delete(hell, "Zipped Successfully.")
...
async def unzip_file(_, message: Message):
    if not message.reply_to_message:
        return await AllAtomic.delete(message, "Reply to a message to unzip it.")

    media = message.reply_to_message.media
    if not media:
        return await AllAtomic.delete(message, "Reply to a media message to unzip it.")

    hell = await AllAtomic.edit(message, "`Unzipping...`")
    start = time.time()
    download_path = await message.reply_to_message.download(
        f"{Config.TEMP_DIR}temp_{round(time.time())}",
        progress=progress,
    ...
    await AllAtomic.delete(hell, "Unzipped Successfully.")
