...
from . import HelpMenu, Symbols, group_only, AllAtomic, on_message
...
async def startvc(client: Client, message: Message):
    if len(message.command) > 1:
        call_name = await AllAtomic.input(message)
    else:
        call_name = "Hellbot VC"

    hell = await AllAtomic.edit(message, "Starting Voice Chat...")
    try:
        await client.invoke(
            CreateGroupCall(
        ...
        await AllAtomic.delete(hell, "Voice Chat started!")
...
async def endvc(client: Client, message: Message):
    hell = await AllAtomic.edit(message, "Ending Voice Chat...")
    ...
    try:
        ...
        await AllAtomic.delete(hell, "Voice Chat ended!")
    except Exception as e:
        await AllAtomic.error(hell, str(e))
...
async def vclink(client: Client, message: Message):
    hell = await AllAtomic.edit(message, "Getting Voice Chat link...")
    ...
    try:
        ...
        await AllAtomic.delete(hell, f"Voice Chat Link: {invite.link}")
    except Exception as e:
...
async def vcmembers(client: Client, message: Message):
    hell = await AllAtomic.edit(message, "Getting Voice Chat members...")
