...
async def videotrim(_, message: Message):
    if message.reply_to_message:
        if not (message.reply_to_message.video or message.reply_to_message.document):
            return await AllAtomic.delete(message, "Reply to a video file")

    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Provide a valid timestamp.")
    ...
    dl_start = time.time()
    hell = await AllAtomic.edit(message, "Downloading...")
    ...
    if not os.path.lexists(file_name):
        return await AllAtomic.error(hell, f"**Error:** `{err}`")
...
async def audiotrim(_, message: Message):
    if message.reply_to_message:
        if not (message.reply_to_message.video or message.reply_to_message.document or message.reply_to_message.audio):
            return await AllAtomic.delete(message, "Reply to a video/audio file")
