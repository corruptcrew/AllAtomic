...
class AtomicClient(Client):
    def __init__(self) -> None:
        ...
        self.bot: Client = Client(
            ...
            plugins=dict(root="AllAtomic.plugins.bot"),
        )
    ...
    async def start_bot(self) -> None:
        ...
        LOGS.info(
            f"{Symbols.arrow_right * 2} Started AllAtomic Client: '{me.username}' {Symbols.arrow_left * 2}"
        )

    async def load_plugin(self) -> None:
        count = 0
        files = glob.glob("AllAtomic/plugins/user/*.py")
        ...
        for file in files:
            with open(file) as f:
                ...
                if shortname in unload:
                    os.remove(Path(f"AllAtomic/plugins/user/{shortname}.py"))
                    continue
                if shortname.startswith("__"):
                    continue
                fpath = Path(f"AllAtomic/plugins/user/{shortname}.py")
                name = "AllAtomic.plugins.user." + shortname
                spec = importlib.util.spec_from_file_location(name, fpath)
                load = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(load)
                sys.modules["AllAtomic.plugins.user." + shortname] = load
                count += 1
            f.close()
        LOGS.info(
    ...
    async def start_message(self, version: dict) -> None:
        await self.bot.send_animation(
            Config.LOGGER_ID,
            "https://i.imgur.com/2Qz9W1K.gif",
            f"**{Symbols.check_mark} AllAtomic is now Online!**\n\n"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton("📦 Repo", url="https://github.com/corruptcrew/AllAtomic"),
                    ],
                    [
                        InlineKeyboardButton("🌸 ComputeCode 🌸", url="https://t.me/ComputeCode"),
                    ],
                ]
            ),
        )
    ...
    async def startup(self) -> None:
        LOGS.info(
            f"{Symbols.bullet * 3} Starting AllAtomic Client & User {Symbols.bullet * 3}"
        )
...
class CustomMethods(AtomicClient):
