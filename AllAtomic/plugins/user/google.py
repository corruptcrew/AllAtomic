...
from . import Config, HelpMenu, Symbols, db, handler, AllAtomic, on_message
...
async def google_search(client: Client, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.edit(message, "Give some text to search on wikipedia.")

    search_query = await AllAtomic.input(message)
    hell = await AllAtomic.edit(
    ...
    try:
        ...
        return await AllAtomic.delete(hell, "**Page not found.**")
...
async def googleSearch(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.edit(message, "Give some text to search on google.")

    search_query = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, f"Searching for `{search_query}` on google...")

    try:
        results = search(search_query, 7, advanced=True)
    except Exception as error:
        return await AllAtomic.error(hell, f"`{str(error)}`")
...
async def gpsLocation(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.edit(message, "Give some place name to search.")
    ...
    if not location:
        return await AllAtomic.delete(hell, "Location not found.")
    ...
    await AllAtomic.delete(hell, f"**🌐 𝖯𝗅𝖺𝖼𝖾:** {address}")


@on_message("webshot", allow_stan=True)
async def webScreenshot(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.edit(message, "Give some url to take screenshot.")

    search_query = await AllAtomic.input(message)
    if not is_valid_url(search_query):
        return await AllAtomic.edit(message, "Invalid url.")

    hell = await AllAtomic.edit(message, f"Taking screenshot of `{search_query}`...")
    driver, resp = Driver.get()
    if not driver:
        return await AllAtomic.error(hell, resp)
...
async def cricketScore(_, message: Message):
    ...
    await AllAtomic.edit(message, final)
...
async def wordMeaning(_, message: Message):
    ...
    if len(message.command) < 2:
        return await AllAtomic.edit(message, "Give some word to search.")

    search_query = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, f"Searching for `{search_query}`...")

    response = requests.get(BASE.format(search_query))
    if response.status_code == 404:
        return await AllAtomic.delete(hell, "Word not found.")
...
async def translateHandler(_, message: Message):
    if message.reply_to_message:
        if len(message.command) < 2:
            return await AllAtomic.edit(message, "Give some language code to translate.")
        toLang = message.command[1]
        text = message.reply_to_message.text or message.reply_to_message.caption
    elif len(message.command) > 2:
        msg = await AllAtomic.input(message)
        toLang = message.command[1]
        text = msg.split(" ", 1)[1]
    else:
    ...
    hell = await AllAtomic.edit(message, f"Translating to `{toLang}`...")
    ...
    try:
        ...
        return await AllAtomic.error(hell, f"`{str(e)}`")
...
async def translateCodes(_, message: Message):
    ...
    await AllAtomic.edit(message, outStr)
...
async def textToSpeech(_, message: Message):
    ...
    try:
        ...
        await message.reply_audio(
            ...
            performer="HellyAI",
            title="Hellbot TTS",
            thumb="./Hellbot/resources/images/AllAtomic_logo.png",
        )
        await hell.delete()
    except Exception as e:
        return await AllAtomic.error(hell, f"`{str(e)}`")
...
async def movieSearch(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.edit(message, "Give a movie name to search on IMDb.")
    ...
    try:
        ...
        await AllAtomic.delete(hell, "No results found.")
    except Exception as e:
...
HelpMenu("google").add(
    ...
    "google", "<query>", "Searches for the given query on google.", "google the AllAtomic"
).add(
    "reverse",
