...
from . import HelpMenu, db, AllAtomic, on_message


@on_message("upload", allow_stan=True)
async def uploadfiles(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Provide a valid file path.")

    query = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, f"**Uploading...** `{query}`")

    if not os.path.exists(query):
        return await AllAtomic.error(hell, f"**Error:** `{query}` **not found.**")
    ...
    try:
        ...
        await AllAtomic.delete(hell, f"**Uploaded** `{query}` **in** `{ul_time}`")
    except Exception as e:
        return await AllAtomic.error(hell, f"**Error:** `{e}`")


@on_message("uploaddir", allow_stan=True)
async def uploadDir(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Provide a valid directory path.")
