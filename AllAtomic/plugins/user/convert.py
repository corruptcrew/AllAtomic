...
from . import HelpMenu, AllAtomic, on_message, Config
...
async def sticker_to_gif(_, message: Message):
    ...
    hell = await AllAtomic.edit(message, "Converting ...")
    ...
    if replied_sticker.is_animated:
        ...
        return await AllAtomic.delete(hell, "Reply to an animated/video sticker.")
    ...
    await AllAtomic.delete(hell, "Converted to gif successfully!")
...
async def sticker_to_image(_, message: Message):
    ...
    await AllAtomic.delete(hell, "Converted to image successfully!")
...
async def image_to_sticker(_, message: Message):
    ...
    await AllAtomic.delete(hell, "Converted to sticker successfully!")
...
async def file_to_image(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.document:
        return await AllAtomic.delete(message, "Reply to a file to convert it to image.")

    if message.reply_to_message.document.mime_type.split("/")[0] != "image":
        return await AllAtomic.delete(message, "Reply to an image file.")
...
async def image_to_file(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await AllAtomic.delete(message, "Reply to an image to convert it to file.")
    ...
    await AllAtomic.delete(hell, "Converted to file successfully!")
...
async def media_to_voice(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await AllAtomic.delete(message, "Reply to a media to convert it to voice.")
    ...
    if os.path.exists(voice_path):
        ...
        await AllAtomic.delete(hell, "Converted to voice successfully!")
        os.remove(voice_path)
    else:
        await AllAtomic.error(hell, f"`{err}`")
...
async def media_to_mp3(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await AllAtomic.delete(message, "Reply to a media to convert it to mp3.")
    ...
    if os.path.exists(mp3_path):
        ...
        await AllAtomic.delete(hell, "Converted to mp3 successfully!")
        os.remove(mp3_path)
    else:
        await AllAtomic.error(hell, f"`{stderr}`")
