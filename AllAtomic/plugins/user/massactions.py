...
from . import HelpMenu, group_n_channel, group_only, AllAtomic, on_message
...
async def banall(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            ...
            return await AllAtomic.error(message, f"__Invalid chatId.__\n\n`{e}`")
    ...
    hell = await AllAtomic.edit(message, f"__Banning all users in {chat_name}.__")
...
async def unbanall(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            chat = await AllAtomic.get_chat(message.command[1])
            chat_id = chat.id
            chat_name = chat.title
        except Exception as e:
            return await AllAtomic.error(message, f"__Invalid chatId.__\n\n`{e}`")

    ban_right = await message.chat.get_member(AllAtomic.me.id)
    if not ban_right.privileges.can_restrict_members:
        return await AllAtomic.delete(
            message,
    ...
    hell = await AllAtomic.edit(message, f"__Unbanning all users in {chat_name}.__")
...
async def kickall(client: Client, message: Message):
    ...
    hell = await AllAtomic.edit(message, f"__Kicking all users in {chat_name}.__")
...
async def deleteall(client: Client, message: Message):
    ...
    hell = await AllAtomic.edit(message, "__Deleting all messages from this user.__")
    user = message.reply_to_message.from_user.id

    await client.delete_user_history(message.chat.id, user)
    await AllAtomic.delete(hell, "__All messages from this user has been deleted.__")
...
async def unblockall(client: Client, message: Message):
    ...
    hell = await AllAtomic.edit(message, f"__Unblocking all users in {chat_name}.__")
...
async def inviteAll(client: Client, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Give a chatId from where to invite all users.")
    ...
    if from_chat.id == -1001641358740:
        return await AllAtomic.delete(message, "Can't add members from this chat!")
    ...
    await AllAtomic.delete(message, f"__Added {len(targets)} users to {to_chat.title}.__")
