...
async def searchImage(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Provide a search query.")
    ...
    if to_send:
        ...
        await AllAtomic.delete(hell, "Uploaded!")
    else:
        await AllAtomic.delete(hell, "No images found.")
...
async def searchWallpaper(_, message: Message):
    ...
    if not access:
        return await AllAtomic.delete(hell, "Unsplash API not found.")
    ...
    if limit > 30:
        return await AllAtomic.delete(hell, "Limit should be less than 30.")
    elif limit < 1:
        return await AllAtomic.delete(hell, "Limit should be greater than 0.")
...
async def glitcher(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await AllAtomic.delete(message, "Reply to a media message to glitch it.")

    hell = await AllAtomic.edit(message, "Glitching...")
    ...
    if media and media not in [
        ...
        return await AllAtomic.delete(hell, "Only media messages are supported.")
    ...
    if dwl_path.endswith(".tgs"):
        ...
        if not os.path.lexists(glitch_img):
            return await AllAtomic.error(hell, f"`{stdout}`\n`{stderr}`")
    elif dwl_path.endswith(".webp"):
        os.rename(dwl_path, glitch_img)
        if not os.path.lexists(glitch_img):
            return await AllAtomic.error(hell, "File not found.")
    ...
    await AllAtomic.delete(hell, f"Glitched at intensity {intensity}")
...
async def deepfry(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await AllAtomic.delete(message, "Reply to a photo to deepfry it.")
    ...
    hell = await AllAtomic.edit(message, "Deepfrying...")
    ...
    await AllAtomic.delete(hell, "Deepfried!")
