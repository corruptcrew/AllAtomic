...
from . import HelpMenu, custom_handler, db, group_n_channel, AllAtomic, on_message
...
async def autopost(client: Client, message: Message):
    ...
    hell = await AllAtomic.edit(message, "Starting Autopost in this group/channel...")

    post_from = message.command[1]
    _chat = await client.get_chat(post_from)

    if not _chat:
        return await AllAtomic.delete(hell, "Invalid chat/channel id.")
...
async def stop_autopost(client: Client, message: Message):
    ...
    hell = await AllAtomic.edit(message, "Stopping Autopost in this group/channel...")
    ...
    if _chat.id not in await db.is_autopost(client.me.id, _chat.id, message.chat.id):
        return await AllAtomic.delete(hell, "This group/channel is not in autopost list.")
...
async def autoposts(client: Client, message: Message):
    hell = await AllAtomic.edit(message, "Getting autopost list...")

    data = await db.get_all_autoposts(client.me.id)
    if not data:
        return await AllAtomic.delete(hell, "No autoposts found.")
    ...
    await AllAtomic.edit(hell, text)
