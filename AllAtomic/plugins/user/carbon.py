...
async def carbon(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Give me some code to make carbon.")

    code = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, "**[ 50% ]** __Making carbon...__")

    driver, resp = Driver.get()
    if not driver:
        return await AllAtomic.error(message, resp)
...
async def karbon(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Give me some code to make karbon.")

    code = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, "**[ 50% ]** __Making karbon...__")
