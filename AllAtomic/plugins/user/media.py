...
async def mediaInfo(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await AllAtomic.delete(message, "Reply to a media file")

    media = message.reply_to_message.media
    hell = await AllAtomic.edit(message, "Getting media info...")
    ...
    if media == MessageMediaType.ANIMATION:
        ...
        return await AllAtomic.delete(message, "Unsupported media type")

    metadata = await get_metedata(media_file)
    if not metadata:
        return await AllAtomic.delete(message, "Failed to get media info")
    ...
    to_paste = f"<strong>💫 AllAtomic Media Info:</strong><br>{metadata}<br><b>📝 MediaInfo:</b><br><code>{out}</code>"
    link = post_to_telegraph("AllAtomicMediaInfo", to_paste)
...
async def memify(_, message: Message):
    if len(message.command) < 2:
        return await AllAtomic.delete(message, "Enter some text!")

    if not message.reply_to_message or not message.reply_to_message.media:
        return await AllAtomic.delete(message, "Reply to a media file")

    start_time = time.time()
    hell = await AllAtomic.edit(message, "Memifying...")
    ...
    if file and file.endswith(".tgs"):
        ...
        if status == False:
            return await AllAtomic.error(hell, pic)
    else:
        return await AllAtomic.delete(message, "Unsupported media type")
    ...
    await AllAtomic.delete(hell, "Done!")
...
async def set_thumbnail(_, message: Message):
    ...
    hell = await AllAtomic.edit(message, "Uploading to telegraph...")
    ...
    await AllAtomic.delete(hell, f"Thumbnail set to [this image]({url})!", 20)
    os.remove(path)
...
async def renameMedia(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await AllAtomic.delete(message, "Reply to a media file to rename it!")
    ...
    new_name = await AllAtomic.input(message)
    hell = await AllAtomic.edit(message, f"Renaming to `{new_name}` ...")
