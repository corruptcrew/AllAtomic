...
async def climate(_, message: Message):
    if len(message.command) < 2:
        ...
        city = await AllAtomic.input(message)

    hell = await AllAtomic.edit(message, f"**Fetching climate data for `{city}`**")
    ...
    if not data:
        return await AllAtomic.delete(hell, f"**Climate data not found for `{city}`**")
...
async def airpollution(_, message: Message):
    ...
    hell = await AllAtomic.edit(message, f"**Fetching air pollution data for `{city}`**")
